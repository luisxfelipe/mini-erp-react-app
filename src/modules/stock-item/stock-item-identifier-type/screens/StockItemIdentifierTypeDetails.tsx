import { useEffect, useState } from 'react';
import { useForm } from 'react-hook-form';
import toast from 'react-hot-toast';
import { z } from 'zod';

import { zodResolver } from '@hookform/resolvers/zod';

import { Input } from '../../../../components/input/Input';
import useStockItemIdentifierTypeRequests from '../hooks/useStockItemIdentifierTypeRequests';
import { IStockItemIdentifierType } from '../interfaces/StockItemIdentifierTypeInterface';

const schema = z.object({
  name: z.string().min(1, 'O campo nome é obrigatório'),
});

type FormData = z.infer<typeof schema>;

interface StockItemIdentifierTypeDetailsProps {
  onCancel: () => void;
  stockItemIdentifierTypeId?: number;
  onSave: () => void;
}

export const StockItemIdentifierTypeDetails = ({
  onCancel,
  stockItemIdentifierTypeId,
  onSave,
}: StockItemIdentifierTypeDetailsProps) => {
  const [stockItemIdentifierType, setStockItemIdentifierType] =
    useState<IStockItemIdentifierType>();
  const { getStockItemIdentifierTypeById, saveStockItemIdentifierType } =
    useStockItemIdentifierTypeRequests();

  const {
    register,
    handleSubmit,
    setValue,
    formState: { errors },
    reset,
  } = useForm<FormData>({
    resolver: zodResolver(schema),
    mode: 'onChange',
  });

  useEffect(() => {
    const loadStockItemIdentifierType = async (id: number) => {
      getStockItemIdentifierTypeById(id)
        .then((stockItemIdentifierTypeLoaded) => {
          if (stockItemIdentifierTypeLoaded) {
            setStockItemIdentifierType(stockItemIdentifierTypeLoaded);
            setValue('name', stockItemIdentifierTypeLoaded.name);
          }
        })
        .catch((error) => {
          throw new Error(`Erro ao buscar o tipo de identificador: ${error}`);
        });
    };

    if (stockItemIdentifierTypeId) {
      loadStockItemIdentifierType(stockItemIdentifierTypeId);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stockItemIdentifierTypeId]);

  function onSubmit(data: FormData) {
    saveStockItemIdentifierType(
      {
        name: data.name,
      },
      stockItemIdentifierTypeId
        ? stockItemIdentifierTypeId.toString()
        : undefined,
    )
      .then(() => {
        onSave();
        toast.success('Tipo de identificador salvo com sucesso!');
        handleCancel();
      })
      .catch((error) => {
        toast.error('Erro ao salvar o tipo de identificador.');
        throw new Error(`Erro ao salvar o tipo de identificador: ${error}`);
      });
  }

  const handleCancel = () => {
    setStockItemIdentifierType(undefined);
    reset();
    onCancel();
  };

  return (
    <div>
      <div className='w-full bg-white p-3 rounded-lg flex flex-col sm:flex-row items-center gap-2 mt-2'>
        <form className='w-full' onSubmit={handleSubmit(onSubmit)}>
          <div className='w-full mb-4'>
            <Input
              className='w-full border-2 rounded-md px-2'
              title='Nome do tipo de identificador'
              type='text'
              placeholder='Digite o nome do tipo de identificador...'
              {...register('name')}
            />
            {errors.name && (
              <p className='my-1 text-red-500'>{errors.name.message}</p>
            )}
          </div>
          <div className='w-96 flex flex-row float-end'>
            <button
              className='w-full ml-2 rounded-md border-2 border-solid '
              type='button'
              style={{ color: '#001529' }}
              color='white'
              onClick={handleCancel}
            >
              Cancelar
            </button>
            <button
              className='w-full ml-2 rounded-md text-white'
              type='submit'
              style={{ backgroundColor: '#001529' }}
              color='white'
            >
              {stockItemIdentifierTypeId ? 'Editar' : 'Inserir'}
            </button>
          </div>
        </form>
      </div>
    </div>
  );
};
